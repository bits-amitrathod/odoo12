...
var DataImport = AbstractAction.extend({
    ...
    init: function (parent, action) {
        ...
        this.res_model =   action.params.model; //action.params[0].request_model;//action.params.model;
        this.parent_context = action.params.context || {};
        // import object id
        this.id = null;
        this.session = session;
        this._title = action.name || _t('Import Template'); // Displayed in the breadcrumbs
        this.do_not_change_match = false;
        this.sheets = [];
        this.selectionFields = {};  // Used to compute fallback values in backend.
        this.customer = action.params.vendor_id;
        this.offer_id = action.params.offer_id;
        this.import_type_ven = action.params.import_type_ven;
        this.parent_model = action.params.model;
        this.upload_document = action.params.upload_document;
...
%(model)s: %(modelValue)s`;
    ...
    */
    ...
    _batchedImport: function (opts, args, kwargs, rec) {
        ...
        return this._rpc({
            model: 'sps.vendor.offer.template.transient',
            method: 'cus_execute_import',
//            args: args.concat([opts],),
            args: [args[0],args[1],args[2], opts, self.parent_model, self.customer, self.template_type, self.upload_document,self.offer_id,self.import_type_ven],
            kwargs: kwargs
        }, {
            shadow: true,
