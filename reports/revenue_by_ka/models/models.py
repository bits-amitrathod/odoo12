...
class RevenueByKa(models.Model):
    ...
    def init_table(self):
        ...
        if start_date_month and end_date_month:
            select_query = """
                SELECT 
                    ROW_NUMBER () OVER (ORDER BY RP.account_manager_cust)           AS id,
                    RP.account_manager_cust                                         AS key_account,
                    SUM(SO.no_of_order)                                             AS no_of_orders,
                    SUM(SOL.revenue)                                                AS total_revenue,

                    """
            select_query = select_query + "SUM(RP.order_quota)*" + str(date_difference) + " AS order_quota, " + \
                           "CASE WHEN SUM(ROUND(RP.order_quota)) > 0 THEN SUM(SO.no_of_order)/SUM(ROUND(RP.order_quota)*" + \
                           str(date_difference) + ")*100 ELSE 0 END AS progress_order_quota," + \
                           "SUM(RP.revenue_quota) *" + str(date_difference) + " AS revenue_quota," + \
                           "CASE WHEN SUM(RP.revenue_quota) > 0 THEN SUM(SOL.revenue)/SUM(RP.revenue_quota*" + str(
                date_difference) + \
                           ")*100 ELSE 0 END AS progress_revenue_quota"
            ...
            select_query = select_query + """
                ...
                LEFT JOIN 
                    (SELECT id, COUNT(sale_order.id) AS no_of_order, sale_order.partner_id 
                        FROM public.sale_order sale_order
                    LEFT JOIN (SELECT DISTINCT ON (origin) origin, date_done, sale_id 
                        FROM stock_picking WHERE picking_type_id = 5 AND state = 'done' ORDER BY origin) AS SP 
                    ON sale_order.id = SP.sale_id
