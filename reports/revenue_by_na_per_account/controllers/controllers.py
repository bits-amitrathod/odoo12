...
class ExportRevenueByNaPerAccount(http.Controller):
    ...
    @http.route('/web/export/revenue_by_na_per_account_export/<string:start_date>/<string:end_date>/<string:national_account_id>',
                type='http',
                auth="public")
    @serialize_exception
    def download_document_xl(self, start_date, end_date, national_account_id, token=1, debug=1, **kw):
        ...
        res = request.make_response(
            self.from_data(["Sale Order#", "Customer Name", "National Account", "Delivery Date", "Total"],
                           records),
            headers=[('Content-Disposition', content_disposition('revenue_by_na_per_account' + '.xlsx')),
