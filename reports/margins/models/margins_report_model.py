...
class MarginsReport(models.Model):
    ...
    def init_table(self):
        ...
        END as margin_percentage, """ + str(group_by) + " as group_by "
        ...
        if not s_date is None:
            date_range = date_range + str(e_date)
        ...
        # include_shipping = self.env.context.get('include_shipping')
        # include_returns = self.env.context.get('include_returns')
        # if not include_returns is None and not include_returns:
