...
class ExportRevenueByBd(http.Controller):
    ...
    @http.route('/web/export/revenue_by_bd_export/<string:start_date>/<string:end_date>/<string:business_development_id>',
                type='http',
                auth="public")
    @serialize_exception
    def download_document_xl(self, start_date, end_date, business_development_id, token=1, debug=1, **kw):
        ...
        if start_date != "all" and end_date != "all":
            select_query = select_query + " AND SPS.date_done BETWEEN '" + str(
        ...
        res = request.make_response(
            self.from_data(["Sale Order#", "Customer Name", "Business Development", "Delivery Date", "Status", "Total"],
                           records),
            headers=[('Content-Disposition', content_disposition('revenue_by_bd' + '.xls')),
