...
class OnHandByDate(models.Model):
 ...
 SELECT  
        ROW_NUMBER () OVER (ORDER BY product_id) as id, p.id as product_id, 
         ...
         t.sku_code as sku_code, b.partner_id as partner_id
 FROM (
        SELECT 
                sq.product_id as product_id, SUM(sq.quantity) as qty_on_hand
        FROM stock_quant sq LEFT JOIN stock_production_lot l ON sq.lot_id = l.id LEFT JOIN stock_location lc ON lc.id = sq.location_id 
        WHERE 
                (lc.create_date <= '""" + str(report_date) + """' OR l.create_date <= '""" + str(report_date) + """') 
                AND sq.quantity >= 0  GROUP BY (sq.product_id)
       ) q 
 FULL JOIN 
