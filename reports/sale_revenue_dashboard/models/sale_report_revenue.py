...
class SaleReport(models.Model):
    ...
    _description = "Sale Revenue Dashboard"
    _auto = False
    _rec_name = 'date'
    _order = 'date desc'
    ...
    user_id = fields.Many2one('res.users', 'Business Development(BD)', readonly=True)
    price_total = fields.Float('Price', readonly=True)
    ...
    key_account = fields.Many2one('res.users', 'Key Account(KA)')
    national_account_rep = fields.Many2one('res.users', 'National Account Rep.(NA)')
    ...
    def _query(self, with_clause='', fields={}, groupby='', from_clause=''):
        ...
        select_ = """
            ...
            s.account_manager as key_account,
            s.national_account as national_account_rep,
            MAX(SP.date_done)  as delivery_date,
            SUM(l.qty_delivered * l.price_reduce)  AS total_amount ,
