...
class ProductTemplateCustomWeb(models.Model):
    ...
    def _get_combination_info(self, combination=False, product_id=False, add_qty=1, pricelist=False, parent_combination=False, only_template=False):
        ...
        test_price_unit = list_price
        if pricelist and product:
            test_price_unit = self.env['account.tax']._fix_tax_included_price_company(
                self.env['sale.order.line']._get_display_price_custom(pricelist, product), product.taxes_id, self.env['account.tax'], self.env['sale.order'].company_id)
