...
class WebsiteSalesReports(http.Controller):
    ...
    @http.route(['/shop/quote_my_report/<int:partner_id>', '/shop/quote_my_report'], type='http', auth="public",
                website=True)
    def quote_my_report(self, partner_id=None):
        _logger.info('In quote my report')

        if not partner_id and request.session.uid:
            # If no partner_id is provided, try to use the logged-in user's partner
            user = request.env['res.users'].sudo().browse(request.session.uid)
            partner_id = user.partner_id.id if user.partner_id and user.partner_id.active else None

        if not partner_id:
            # Redirect if partner_id is still not resolved
            ...
            if user and user.partner_id and user.partner_id.active and user.partner_id.id == partner_id:
                ...
                try:
                    ...
                    if user and user.share is False and user.partner_id and user.partner_id.active is False:
                        return request.render('website_sales.quote_my_report', {
                            'active': False,
                            'product_list': {},
                            'product_sorted_list': {}
                        })
                        ...
                        product_list, product_sorted_list = request.env[
                            'quotation.product.list'].sudo().get_product_list(partner_id)
                        return request.render('website_sales.quote_my_report', {
                            'active': True,
                ...
                return request.render('website_sales.quote_my_report', {'invalid_url': invalid_url})
        else:
            portal_url = \
                partner.with_context(signup_force_type_in_url='', lang=partner.lang)._get_signup_url_for_action()[
                    partner.id]
            if portal_url:
